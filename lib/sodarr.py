...
def get_sonarr_library():
	...
	r = requests.get(config.sonarr_host + '/api/v3/series', headers=headers, timeout=60)
	try:
...
class API(object):
	...
	# DOES NOT WORK
	def get_wanted_missing(self, page=1):
		"""Gets missing episode (episodes without files)"""
		res = self.request_get("{}/wanted/missing/?page={}".format(self.host_url, page))
		return res.json()
	...
	def get_tv(self):
		res = self.request_get("{}/tv".format(self.host_url))
		return res.json()
	...
	def upd_movie(self, data):
		res = self.request_put("{}/movie".format(self.host_url), data)
		return res.json()
